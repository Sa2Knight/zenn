...
そのようなコンポーネントは、ある端末見た時は正常に描画されているが、別な画面サイズの端末で見ると崩れてしまっているということも多いでしょう。
...
`MyPage` コンポーネントは `props` / `emits` を特に持たないため、以下のようなシンプルなストーリーから初めます。
...
```bash
$ yarn add -D @storybook/addon-viewport@7.0.2
```
...
多くの場合はこの３種類があれば充分ですが、各種端末別の画面サイズで検証したいケースも考えられます。
...
これで `iPhone12` や `Pixel` など、実際の端末に基づいたビューポートが選択できるようになりました。
...
ツールバーから Viewport を切り替えるのも良いですが、都度マウス操作するのも億劫です。

ストーリーごとに使用するビューポートを設定することで、各ビューポートでのレイアウトを確認を容易に出来ます。
