...
本記事では、メタ社(旧 FaceBook) が開発した OSS である [memlab] をさっそく試してみようという記事です。

公式ドキュメント以上の付加価値はあまりありませんが、ざっくりと雰囲気を掴んでもらって使用を検討してもらえれば幸いです。

# memlabについて

`memlab` は、 [Puppeteer](https://pptr.dev/) API を用いたシナリオを作成することで、そのシナリオ実行によって発生するメモリリークの検出及び検出の補助を行ってくれるツールです。

https://facebookincubator.github.io/memlab/

本記事では実際に動かすところに重きを置くので、ツールの背景などは以下記事を参照ください。

https://www.publickey1.jp/blog/22/javascriptmemlab.html

`memlab` では概ね以下のことが行なえます。

- `Puppeteer` ベースでの宣言的なシナリオの作成
- ヒープ領域とメモリリークの可視化
  - シナリオ内でのヒープのスナップショットを都度取得
...
従来は SPA の画面遷移を手動で繰り返し行い、ブラウザのデベロッパーツールでメモリの増減を確認してメモリリークがないかを確認していた作業を簡易的に自動化することができます。
